...
export default function Listings() {
  ...
  useEffect(() => {
    ...
    // ✅ Use router.replace() to avoid infinite reloads
  ...
  useEffect(() => {
    async function fetchListings() {
      ...
      try {
        ...
        // ✅ Fetch sellers only once to prevent API spam
  ...
  const removeFilter = (key) => {
    setFilters((prevFilters) => {
      const updatedFilters = { ...prevFilters, [key]: "" }; // ✅ Reset filter
      return updatedFilters;
    });
  
    // ✅ Correctly update URL after removing a filter
    const params = new URLSearchParams();
    Object.keys(filters).forEach((filterKey) => {
      if (filters[filterKey]) params.set(filterKey, filters[filterKey]);
    });
